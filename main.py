...
def main():
    ...
    #nome_banco_relacional = 'NORTHWND'
    nome_banco_relacional = 'AdventureWorks2022'
    nome_banco_grafo = 'teste1' 

    [metadata, conn] = obter_metadata_banco(nome_banco_relacional)
    
    estruturas_tabelas = obter_estruturas_relacional(metadata)
            
    gerar_output_json(nome_banco_relacional, estruturas_tabelas, "[1] METADADO - estruturas_tabelas_inicial.json")
    
    estruturas_tabelas = aninhar_relacoes(estruturas_tabelas)

    gerar_output_json(nome_banco_relacional, estruturas_tabelas, "[2] METADADO - estruturas_tabelas_pre_aninhar.json")

    parametros_tabela_aninhado = gerar_parametros_aninhados(estruturas_tabelas, conn)

    gerar_output_json(nome_banco_relacional, parametros_tabela_aninhado, "[3] VALORES - parametros_tabela_aninhado.json")

    novas_tabelas = reescrever_metadados_com_aninhamento(estruturas_tabelas)
    
    gerar_output_json(nome_banco_relacional, novas_tabelas, "[4] METADADO - estruturas_tabelas_aninhadas.json")
    ...
    gerar_output_json(nome_banco_relacional, grafo_generico, "[5] GRAFO COMPLETO - grafo_generico.json")
    gerar_output_json(nome_banco_relacional, nos_generico, "[5] NOS - nos_generico.json")
    gerar_output_json(nome_banco_relacional, arestas_generico, "[5] ARESTAS - arestas_generico.json")

        
    vertices_janus_teste = gerar_vertice_janus(nos_generico)
    
    gerar_output_json(nome_banco_relacional, vertices_janus_teste, "[6] JANUS - vertices_janus_teste.json")

    edges_janus_teste = gerar_edges_janus(arestas_generico)
    
    gerar_output_json(nome_banco_relacional, edges_janus_teste, "[6] JANUS - edges_janus_teste.json")
    driver = get_driver()
    import_data(driver, grafo_generico, nome_banco_grafo)
    driver.close()
